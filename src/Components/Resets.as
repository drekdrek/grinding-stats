		...
		while (this.running) {
			...
			if (playground !is null && playground.GameTerminals.Length > 0) {
				...
				if (gui_player !is null) {
					auto post =
						(cast<CSmScriptPlayer>(gui_player.ScriptAPI)).Post;
				...
				if (gui_player !is null) {
					...
					if (!handled &&
						race_state ==
							CTrackManiaPlayer::ERaceState::BeforeStart) {
						handled = true;
						session += 1;
						total += 1;
					}
				...
				if (gui_player !is null) {
					auto ui_sequence =
						(cast<CGamePlaygroundUIConfig>(
							 network.PlaygroundClientScriptAPI.UI))
							.UISequence;
					...
					if (handled &&
						race_state !=
							CTrackManiaPlayer::ERaceState::BeforeStart &&
						(race_state !=
							 CTrackManiaPlayer::ERaceState::Finished &&
